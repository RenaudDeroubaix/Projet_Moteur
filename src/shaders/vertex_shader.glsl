...
layout(location = 1) in vec3 vertices_position_modelspace;
...
void main(){
    ...
    mat4 MVP = projmat * MV ;
    light_pos_worldspace =  modelmat * vec4(light_pos,1.f);
    //light_dir_worldspace = MV * vec4(light_dir,0.f);
    normal_surface =  normalize(MV  * vec4(normals , 0.f));
    tex_coord = uv_coords;
    
    // TODO : Output position of the vertex, in clip space : MVP * position
    position_worldspace = MV * vec4(vertices_position_modelspace , 1.f);
