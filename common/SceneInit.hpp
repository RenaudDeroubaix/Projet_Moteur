...
void makeScene_0(Scene* s, const unsigned int SCR_WIDTH, const unsigned int SCR_HEIGHT){
    ...
    //////////////ROOM 
    Node* sol = makeRoom(*s,36,60,6,glm::vec3(1.0f));   
   
   //////////////PLAYER
    Node* player = s->make_node_cube();
    ...
    Node* SecurityCam1 = s->make_node_camera(false,SCR_WIDTH , SCR_HEIGHT);
    ...
    Node* SecurityCam3 = s->make_node_camera(false,SCR_WIDTH , SCR_HEIGHT);
    ...
    Node* obstacle1 = s->make_node_cube();
    ...
    Node* eventPorte=s->make_node_event(typeEvent::TP_Scene_Forward,glm::vec3(0.f,playerScale/2.0,0.f));
